  ...
  for(j in (i+1):N){
    if(adj_m[i,j] == 1){
      ...
      if((group_i == group_j) & (group_j == 1)){
        ...
        w_loc <- sample(1:length(t4.s), size = 5, replace = TRUE)
        words = t4.s[w_loc]
        result_l[count, 3] = paste(words,collapse = ' ')
      }
      count = count + 1
...
write.table(group_l, file = 'sim_A_0214_group.csv', row.names=FALSE,col.names=FALSE, sep=",")
result_l

write.table(result_l, file="60_sim_A_0214.csv",row.names=FALSE,col.names=FALSE, sep=",")


adj_m[lower.tri(adj_m)] <- t(adj_m)[lower.tri(adj_m)]
write.csv(adj_m, file="60_sim_A_0214_adjm.csv", )
...
write.table(wordmat, file="0214_sim_A_wordmat.csv",row.names=FALSE,col.names=FALSE, sep=",")
